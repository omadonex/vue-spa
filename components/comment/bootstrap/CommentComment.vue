  ...
  <div class="dx-comments__comment clear-fix">
    ...
    <div class="dx-comments__body">
      <div class="subheading">
        <span v-if="currentUserOwner"><i class="bi-bookmark-fill"></i></span>
        <span :class="['primary--text', { 'ml-1' : currentUserOwner }]">{{ commentData.user.displayName }}</span>
        <span class="ml-1"><i class="bi-circle" style="font-size: 12px;"></i></span>
        <span class="ml-1 caption">{{ commentData.commentedAtStr }}</span>
      </div>
      ...
      <div v-else>
        <textarea
            ...
            rows="3"
            :label="t('formLabelEdit')"
            v-model="commentText"
        ></textarea>
        <button
            @click="commentUpdate" :loading="loading" color="blue-grey"
      ...
      <div class="pt-3" v-else-if="replyFormVisible">
        <comment-sign-in></comment-sign-in>
      </div>
    </div>
  </div>
</template>
